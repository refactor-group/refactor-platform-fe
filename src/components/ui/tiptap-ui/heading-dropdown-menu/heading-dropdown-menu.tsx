...
} from "@/components/ui/tiptap-ui/heading-button/heading-button"
...
export function HeadingDropdownMenu({
  ...
  const isDisabled = !(editorState?.canToggle ?? false);
  const isAnyHeadingActive = editorState?.isActive ?? false;
  ...
  const show = React.useMemo(() => {
    ...
    if (hideWhenUnavailable) {
      if (isNodeSelection(editor.state.selection) || !(editorState?.canToggle ?? false)) {
        return false
      }
    }

    return true
  }, [headingInSchema, editor, hideWhenUnavailable, editorState?.canToggle])
