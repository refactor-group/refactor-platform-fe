...
import { ZoomDisconnectDialog } from "@/components/ui/settings/zoom-disconnect-dialog";

describe("ZoomDisconnectDialog", () => {
  const defaultProps = {
    onConfirm: vi.fn(),
  };
  ...
  it("opens dialog when Disconnect button is clicked", async () => {
    render(<ZoomDisconnectDialog {...defaultProps} />);
    ...
    expect(
      screen.getByText("Disconnect Zoom Account")
    ).toBeInTheDocument();
  });
